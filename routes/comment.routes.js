...
router.post('/posts/:postId/comment', (req, res, next) => {
    ...
    User.findOne({ username: explorer})
      .then(userDocFromDB => {
        user = userDocFromDB;
  
        // 1. if commenter is not user yet, let's register him/her as a user
        if (!userDocFromDB) {
          return User.create({ username: explorer });
        }
      })
      .then(newUser => {
        // prettier-ignore
        ...
        .then(dbPost => {
          ...
          if (newUser) {
            newComment = new Comment({ explorer: newUser._id, content });
          } else {
            newComment = new Comment({ explorer: user._id, content });
          }
